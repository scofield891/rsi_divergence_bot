...
def in_pullback_zone(close_val, ema20, ema50, atr_val, tol_atr=0.10, direction='long'):
    ...
    if direction == 'long':
        ...
        return (close_val < ema20) and (lo <= close_val <= hi)  # Stricter for short: < ema20
...
async def scan_symbol(symbol):
    try:
        ...
        if not direction:
            ...
            return
        # Cooldown kontrol
        key = f"{symbol}"  # Per-symbol cooldown, direction-independent
...
def backtest_symbol(symbol, bars_2h=800):
    ...
    try:
        ...
        for i in range(i_start, len(df2)-2):
            ...
            if not direction:
                continue
            # kabaca: TP1 veya TP2 ilk 40 bar içinde dokunursa win say
