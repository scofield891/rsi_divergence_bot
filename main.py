...
exchange = ccxt.bybit({'enableRateLimit': True, 'options': {'defaultType': 'futures'}}) # Futures için değiştirildi
...
sorted_markets = sorted(futures_markets, key=lambda x: x['info'].get('24h_volume', 0), reverse=True)
...
return ['PERP/USDT', 'BTC/USDT'] # Hata durumunda fallback
...
bullish = False # Pozitif uyumsuzluk (bullish)
bearish = False # Negatif uyumsuzluk (bearish)
...
await asyncio.sleep(1) # Rate limit için 1 saniye delay
